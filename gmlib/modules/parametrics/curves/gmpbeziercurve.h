...
namespace GMlib {
  ...
  public:
    ...
    void            moveLocalCoordinatsystem(const Vector<T,3>& cm);
    ...
    // from SceneObject
    // The functions below are not meant for public use, it is for editing on the curve
    ...
    Point<T,3>      getSamplePoint(int i, int j) const override;
    Vector<T,3>&    getSampleDerivative(int i, int j, int d) const override;
    int             getSample_d(int i, int j) const;
...
//    void            resample( std::vector<DVector<Vector<T,n>>>& p, Sphere<T,3>& s, const std::vector<T>& t, int d ) const override;
    ...
    // Virtual functions from SceneObject, which we implemented locally because of use of Bezier-curves as local curves in Blending-curves
