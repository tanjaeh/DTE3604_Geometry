...
//*****************************************
   ...
   *  \param cu_len Whether we use uniform sampling (false - default value), or curve length sampling (true)
   */
  template <typename T>
  inline
  PERBSCurve<T>::PERBSCurve(PCurve<T,3>* g, int n, bool cu_len) {
  ...
  void PERBSCurve<T>::edit( SceneObject *obj )
  {
      if(obj->getName() == _origin->getName()) {
          ...
          if(_origin->isClosed() != this->isClosed()) { // from closed to open etc.
              _origin->sample(m, d);
...
//    prepareSampling(d);
  ...
  void  PERBSCurve<T>::preSample( int d ) {
      ...
      // for hver partisjon -
      for(unsigned int i=0; i<_pre_basis.size(); i++) {             // For each partition
          ...
          for(unsigned int j=0; j<_pre_basis[i].size(); j++) {      // for hvert sample-punkt i partisjonen
  ...
  /*! void  PERBSCurve<T>::updateSamples() const
   *  \brief Private, not for public use
   *
  ...
  void  PERBSCurve<T>::updateSamples() const {
      if(_local_change.size() > 0) {
          ...
          for(unsigned int i=0; i<ind.size(); i++) // for all partisions
          ...
          for(unsigned int i=0; i<this->_visu.size(); i++) {  // for all partisions
   ...
   */
    ...
    template <typename T>
    inline
    void PERBSCurve<T>::localSample( const typename PCurve<T,3>::Sampler& sample) const {

 //std::cout.precision(2);
 //std::cout << "Inkommende sampler er: " << sample << std::endl;
        for (int i=0; i<_c.getDim(); i++) {
            std::vector<T> tmp;
            T s = _c[i]->getParStart();
            T e = _c[i]->getParEnd();
            T sp;
            if(s < this ->getParStart())
                sp = s + this->getParDelta();
            else {
                sp = s;
            }
          //  _c[i]->
// std::cout << "Bezierkurve " << i << ", start:  \t" << s << " end:  " << e << std::endl;
            if(this->isClosed() && s < this->getParStart()) {   // Closed curve
                for (int j=0; j<sample.size(); j++) {
                    if(sample[j].back() <= sp) continue;        //Skips those who do not reach local start
                    int k = sample[j].size()-1;                 // Index of last sample value
                    for(; k>0; k--) if (sample[j][k-1] < sp) break;
                    for(; k<sample[j].size(); k++) tmp.push_back(sample[j][k]-this->getParDelta());
                }
            }

            for (int j=0; j<sample.size(); j++) {
                if(sample[j][0] >= s && sample[j][0] <= e) {
                    if(tmp.empty())
                        tmp.push_back(sample[j][0]);
                    else if(this->isClosed() && tmp.back() == this->getParEnd())
                        tmp.back() = this->getParStart();
                }

                for (int k=1; k<sample[j].size(); k++)
                    if(sample[j][k] >= s && sample[j][k] <= e)
                        tmp.push_back(sample[j][k]);
            }
 //std::cout << "tmp " << tmp << std::endl;

 //           _c[i]->prepareForSampling();
            _c[i]->preSample(tmp, this->_visu.no_derivatives, 1);

            std::vector<int> trans_pre(sample.size(),0);
            for (int j=0; j<sample.size(); j++) {
                if(tmp[0] == sample[j][0])
                    trans_pre[j] = 0;
                else {
                    ...
                    for (int k=1; k<sample[j].size()-1; k++)
                        if(tmp[0] == sample[j][k])
                            trans_pre[j] = -k;
                }
            }
            _c[i]->setIndPreEval(trans_pre);
// std::cout << "Trans pre " << trans_pre  << "\n" << std::endl;
        }
    }
